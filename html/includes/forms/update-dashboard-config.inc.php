...
if ($sub_type == 'remove' && is_numeric($widget_id)) {
    if (dbFetchCell('select 1 from dashboards where (user_id = ? || access = 2) && dashboard_id = ?', array(Session::get('user_id'),$dasboard_id)) == 1) {
    ...
    if (dbFetchCell('select 1 from dashboards where (user_id = ? || access = 2) && dashboard_id = ?', array(Session::get('user_id'),$dasboard_id)) == 1) {
        ...
        if (is_array($widget)) {
            ...
            $item_id = dbInsert(array('user_id' => Session::get('user_id'),'widget_id'=>$widget_id, 'col'=>1,'row'=>1,'refresh'=>60,'title'=>$widget['widget_title'],'size_x'=>$x,'size_y'=>$y,'settings'=>'','dashboard_id'=>$dasboard_id), 'users_widgets');
